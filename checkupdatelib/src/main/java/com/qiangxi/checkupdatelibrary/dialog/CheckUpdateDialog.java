...
class InternalDialog extends Dialog implements DownloadCallback {
    ...
    InternalDialog(@NonNull Context context, CheckUpdateDialog checkUpdateDialog) {
        super(context, (checkUpdateDialog.mOption.getMode() == 2) ? R.style.checkUpdateDialogStyleTwo : R.style.checkUpdateDialogStyle);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mOption.getMode() == 1) {
            // 模式一
            View layout = LayoutInflater.from(getContext()).inflate(R.layout.dialog_check_update_layout, null);
            setContentView(layout);
        } else {
            // 模式二
            View layout = LayoutInflater.from(getContext()).inflate(R.layout.dialog_check_update_two_layout, null);
            setContentView(layout);
        }
                ...
                if (mActivity.getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.M) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        ...
                        if (permissionStatus != PackageManager.PERMISSION_GRANTED) {
                            mCheckUpdateDialog.requestPermissions(new String[]{Manifest.permission.READ_EXTERNAL_STORAGE,Manifest.permission.WRITE_EXTERNAL_STORAGE}, 0x007);
                        } else {
